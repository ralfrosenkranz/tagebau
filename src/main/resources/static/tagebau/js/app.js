(function(){
  ...
  function fmtMoney(amount, currency){
    if(amount===undefined||amount===null || amount==="") return ""; //"629500000";
    var n = Number(amount); 
  ...
  async function initLanding(){
    ...
    try{
      ...
      if(landing && landing.hero){
        setText(qs('[data-hero-kicker]', root), landing.hero.kicker);
        setText(qs('[data-hero-title]', root), landing.hero.title);
        setText(qs('[data-hero-text]', root), landing.hero.text);
      }
  ...
  async function initProduct(){
    ...
    try{
      ...
      if(prod){
        setText(titleEl, prod.nickname || prod.technicalName || prod.id || "Produkt");
        ...
        if(priceEl) priceEl.textContent = fmtMoney(amount, currency);
        if(descEl) descEl.textContent = (prod.description || prod.technicalName || "");
      }
    }catch(e){
      ...
      try{
        ...
        if(Array.isArray(imgs) && imgs.length){
          ...
          if(urls.length){
            ...
            if(main) main.setAttribute("src", urls[0]);
  ...
  function renderCart(cart){
    ...
    if(subtotalEl) subtotalEl.textContent = fmtMoney(subtotal, currency);
